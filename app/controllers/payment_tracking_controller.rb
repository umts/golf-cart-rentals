...
class PaymentTrackingController < ApplicationController
  ...
  def index
    ...
    # negative 1 is default because even paid rentals wont have negative balance
    min = (params.permit(:balance_gteq)[:balance_gteq] || -1).to_f
    ...
    if search_q[:created_at_gteq].present?
      search_q[:created_at_gteq] = Date.parse(search_q[:created_at_gteq]).beginning_of_day
    end
    if search_q[:created_at_lteq].present?
      search_q[:created_at_lteq] = Date.parse(search_q[:created_at_lteq]).end_of_day
    end
  ...
  def send_invoice
    ...
    InvoiceMailer.send_invoice(rental).deliver_later if check_permission_for(rental)
  end
